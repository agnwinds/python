...
python_objects = bb.o get_atomicdata.o python.o photon2d.o photon_gen.o \
		...
		sv.o ionization.o  ispy.o  pfop.o levels.o gradv.o reposition.o \
		...
		cylind_var.o bilinear.o gridwind.o

# Note python_source differs slightly from python_objects.  It omits pfop because
# that is a fortran file and get_models.c because that has a special .h file of its
# own that can interfere with python.h 
...
python_source= bb.c get_atomicdata.c python.c photon2d.c photon_gen.c \
		...
		cylind_var.c bilinear.c gridwind.c

additional_py_wind_source = py_wind_sub.c py_ion.c py_write.c

prototypes: 
	cp templates.h templates.h.old
	cproto -I$(INCLUDE)  -I$(INCLUDE2) -U main $(python_source) ${additional_py_wind_source} > foo.h      
	cp foo.h templates.h

python: startup  $(python_objects)
	gfortran  ${CFLAGS} $(python_objects) $(LDFLAGS) -o python
		cp $@ $(BIN)/py
		mv $@ $(BIN)/py$(VERSION)


py_wind_objects = py_wind.o get_atomicdata.o py_wind_sub.o windsave.o py_ion.o \
		...
		lines.o vector.o wind2d.o wind.o  ionization.o pfop.o py_write.o levels.o \
...
py_wind: startup $(py_wind_objects)
	gfortran $(CFLAGS) $(py_wind_objects) $(LDFLAGS) -o py_wind
	cp $@ $(BIN)
...
test_bb: bb.o test_bb.o pdf.o recipes.o bilinear.o time.o 
	gfortran  ${CFLAGS} bb.o pdf.o test_bb.o  recipes.o bilinear.o time.o $(LDFLAGS) -o test_bb
...
py_ray: bb.o get_atomicdata.o py_ray.o photon2d.o photon_gen.o \
		...
		yso.o cylindrical.o rtheta.o \
		matom.o estimators.o wind_sum.o
	gfortran  ${CFLAGS} py_ray.o get_atomicdata.o photon2d.o photon_gen.o \
		bb.o detail.o \
		saha.o spectra.o wind2d.o wind.o  vector.o debug.o recipes.o \
		trans_phot.o phot_util.o resonate.o radiation.o \
		...
		sv.o proga.o knigge.o disk.o pfop.o levels.o gradv.o density.o \
		yso.o cylindrical.o rtheta.o \
		matom.o estimators.o wind_sum.o \
		$(LDFLAGS) -o py_ray
		mv $@ $(BIN)/py_ray
...
py_grid: bb.o get_atomicdata.o py_grid.o photon2d.o photon_gen.o \
	...
	gfortran  ${CFLAGS} py_grid.o get_atomicdata.o photon2d.o photon_gen.o \
		bb.o\
		saha.o spectra.o wind2d.o wind.o  vector.o debug.o recipes.o \
		trans_phot.o phot_util.o resonate.o radiation.o \
		stellar_wind.o   corona.o util.o anisowind.o \
...
balance: balance.o balance_sub.o balance_gen.o balance_abso.o \
		...
		roche.o stellar_wind.o  proga.o corona.o disk.o pfop.o knigge.o thierry.o \
		lines.o get_atomicdata.o random.o wind2d.o wind.o   bal_photon2d.o  levels.o  \
		util.o anisowind.o reposition.o density.o  detail.o bands.o  
	gfortran   ${CFLAGS} balance.o balance_sub.o balance_gen.o   balance_abso.o \
		...
		extract.o ispy.o roche.o stellar_wind.o proga.o corona.o disk.o pfop.o knigge.o  \
		util.o anisowind.o reposition.o density.o bands.o \
		$(LDFLAGS) -o balance
	mv $@ $(BIN)/balance
#	mv $@ $(BIN)/balance44
