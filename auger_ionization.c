  ...
  for (n = 0; n < nauger; n++)
    {
	 ...
	 manner
      ...
      rad_rec_rate_coeff = augerion[n].arad * pow( xplasma->t_e/10000.,(-1.*augerion[n].etarad));
      diel_rec_rate_coeff = augerion[n].adi * pow( (xplasma->t_e), -1.5) * exp(-1. * augerion[n].t0di / xplasma->t_e) * ( 1. + (augerion[n].bdi * exp(-1.*augerion[n].t1di / xplasma->t_e)));


      if ((augerion[n].nion_target != -1) && (xplasma->density[augerion[n].nion] > DENSITY_MIN))
	{
	  if ((factor = (ionization_rate_coeff / (rad_rec_rate_coeff + diel_rec_rate_coeff) / xplasma->ne)) < 1.0)
	    {
	      ...
	      xplasma->density[augerion[n].nion_target] = xplasma->density[augerion[n].nion] * factor;
	      //printf("New value: %g %g\n", xplasma->density[augerion[n].nion_target], xplasma->density[augerion[n].nion]);
	    }
	  else
	    {
	      Error("Auger_ionization trying to set large upper population. Setting equal.\n");
	      xplasma->density[augerion[n].nion_target]=xplasma->density[augerion[n].nion];
	    }
