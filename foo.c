...
/* Next three lines have variables that should be a structure, or possibly we should allocate the space for the spectra to avoid
   all this nonsense.  02feb ksl */
    ...
    double          tmax;
    char            root[LINELENGTH], input[LINELENGTH], wspecfile[LINELENGTH], specfile[LINELENGTH], diskfile[LINELENGTH];
    char            windradfile[LINELENGTH], windsavefile[LINELENGTH];
    char            specsavefile[LINELENGTH];
    char            photfile[LINELENGTH], diagfile[LINELENGTH], old_windsavefile[LINELENGTH];
    ...
    double          time_max;
    //The maximum time the program is allowed to run before halting
    ...
    verbosity = 4;		/* Set the default verbosity to 4.  To get more info raise the verbosity level to a higher number.
...
/* 0811 - ksl - If the restart flag has been set, we check to see if a windsave file exists.  If it doues we will we will restart
   from that point.  If the windsave file does not exist we will start from scratch */
...
/* Start logging of errors and comments */
    ...
    //54f-- ksl-- Now read from version.h
...
/* Now create the names of all the files which will be written.  Note that some files have the same root as the input file, while
   others have a generic name of python. This is intended so that files which you really want to keep have unique names, while
   those which are for short-term diagnostics are overwritten.  ksl 97aug. */


    strcpy(basename, root);
    //56 d-- ksl-- Added so filenames could be created by routines as necessary
   ...
   system */
	...
	/* This option is for the confusing case where we want to start with a previous wind model, but we are going to write the
	   result to a new windfile */
	    ...
	    wind_read(old_windsavefile);
	    //program will exit if unable
		to read the file
		    geo.wind_type = 2;
	    //after wind_read one will have a different wind_type otherwise
		w = wmain;


	} else
	{			/* Read the atomic datafile her, because for the cases where we have read and old wind files, we
...
NPHOT = photons_per_cycle;
//For now set NPHOT to be be photons / cycle-- > subcycles == 1
...
/* Normally, geo.partition_mode is set to -1, which means that partition functions are calculated to take full advantage of the
   data file.  This means that in calculating the partition functions, the information on levels and their multiplicities is taken
   into account.   */

geo.partition_mode = -1;
//? ? Stuart, is there a reason not to move this earlier so it does not affect restart
...
/* ?? Next section seems rather a kluge.  Why don't we specifty the underlying variables explicitly It also seems likely that we
   have mixed usage of some things, e.g ge.rt_mode and geo.macro_simple */

/* For now handle scattering as part of a hidden line transfermode ?? */
if (geo.line_mode == 4)
{
    geo.scatter_mode = 1;
    //Turn on anisotropic scattering
	geo.line_mode = 3;
    //Drop back to escape probabilities
	geo.rt_mode = 1;
    //Not macro atom(SS)
} else if (geo.line_mode == 5)
{
	...
	geo.line_mode = 3;
    //Single scattering model is best for this
	mode
	    geo.rt_mode = 1;
    //Not macro atom(SS)
} else if (geo.line_mode == 6)
{
    ...
    //Single scattering
	geo.rt_mode = 2;
    //Identify macro atom treatment(SS)
	geo.macro_simple = 0;
    //We don 't want the all simple case (SS)
...
/* 57h -- Next line prevents bf calculation of macro_estimaters when no macro atoms are present.   */
    ...
    if (nlevels_macro == 0)
	geo.macro_simple = 1;
    //Make everything simple if no
	macro atoms-- 57 h

	    // SS - initalise the choice of handling for macro
	    pops.
		if (geo.wind_type == 2)
	    {
		geo.macro_ioniz_mode = 1;
		//Now that macro atom properties are available for restarts
		    }
		    else
		{
	    ...
	    /* 080517 - ksl - Reassigning bb to -1, etc is to make room for reading in model grids, but complicates what happens if
	       one tries to restart a model.  This needs to be updated so one can re-read the geo file, proabbly by defining
	       variaables BB etc, and then by checking whether or not the type is assigned to BB or read in as 0. Also need to
	       store each of these model list names in geo structure. */
		...
		if (geo.wind_type != 2)
		    //Start of block to define a model for the
			first time
		    {
			...
			if (geo.binary_system)
			{
			    ...
			    geo.m_sec /= MSOL;
			    //Convert units for ease
				of data entry
				    rddoub("msec(msol)", &geo.m_sec);
			    geo.m_sec *= MSOL;

			    geo.period /= 3600.;
			    //Convert units ro hours for easy
				of data entry
				    rddoub("period(hr)", &geo.period);
			    geo.period *= 3600.;
			    //Put back to cgs immediately
			}
		    /* Describe the disk */
			...
			if (geo.disk_type)	/* Then a disk exists and it needs to be described */
			{
				...
				//Convert to msol / yr to simplify input
			...
			/* 04aug ksl ??? Until everything is initialized we need to stick to a simple disk, while teff is being set
			   up..  This is because some of the models, e.g. knigge have wind structures that depend on teff.
			
			080518 - ksl - this is quite confusing.  I understand that the KWD models have base velociites that are
			   affected by t_eff, but we have not done anything yet.  Possible this is a consideration for restart, but
			   I would have guessed we recalculated all of that, and in any event this is within the block to reset
			   things.  this is likely a problem of some sort
			
			However, the next line does force the illum to 0 while initialization is going on, unless ?? the illum is 3 */

			    geo.disk_illum = 0;
			    if (disk_illum == 3)
				//080518 - And why is it different in the analytic case ?
			    {
				geo.disk_illum = 3;
			    }
		    ...
		    Note: When one adds a new model, the only things that should be read in and modified are parameters in geo.
		       This is in order to preserve the ability to continue a calculation with the same basic wind geometry,
		       without reading in all of the input parameters.  */
	    ...
	    /* Now define the wind cones generically.  Note that thetamin and thetamax are measured from disk plane, and so if
	       thetamin = 90 one ends up with a cone that is constant in rho.  56d -- ksl -- updated windcone definitions  */
	    /* ???? There is some confusion here regarding new and old.  This should be ???? fixed.  ksl */
	    ...
	    /* Check that the parameters which have been supplied for the star, disk and boundary layer will allow generation of
	       photons where that is appropriate */
		...
		/* SS June 04: convert these to frequencies and store for use in computing macro atom and k-packet emissivities. */
		    ...
		    //initialise for ionization
			cycles - don 't use pre-computed emissivities for macro-atom levels/ k-packets.
		...
		/* 05apr-ksl-56--For diagnositic reasons I have left questions regarding phase even for systems which are not
		   binaries.  Phase 0 in this case corresponds to an extraction direction which is in the xz plane */
	    ...
	    /* 081221 - 67c - Establish limits on the frequency intervals to be used by the ionization cycles and the fraquency
	       bands for stratified sampling.  Changes here were made to allow more control over statified sampling, since we have
	       expanded the temperature ranges of the types of systems we would like to calculate.  This section of inputs might
	       logically go earlier in the code, but was put here so it would add on to existing .pf files.  It would be reasonble
	       to consider moving it to a more logical location */
		...
		if (geo.wind_type != 2)
		    //Define the wind and allocate the arrays the first time
			define_wind();
		//Do not reinit if you
		    want to use old windfile
	    ...
	    /* The next section sets up a structure qdisk to record the effects of illumination on the disk.  disk_init is called
	       primarily to get a defined set of annular rings which are kept throughout the ionization calculation.  A second
	       structure qdisk is needed because in the process of generating photons in various bands the annular rings are
	       changed
	    
	    disk_init calculates the flux from the disk in the energy range set by freqmin and freqmax, and uses is this to
	       identify the position of the rings in the disk, so that each ring contributes the same amount to the flux
	    ...
	    */
	    ...
	    080518 - ksl - I believe that the reason for this somewhat weird logic is to assure that models (e.g corona and knigge)
	       where the base wind velocity depends on teff are not altered by illumination, but since no photons have been
	       transported at this stage, it's hard to see why that would matter. */
		...
		//SS July 04 - want to compute MC estimators during ionization cycles
		...
		/* 080520 - ksl - There is a conundrum here.  One should really zero out the quantities below each time the wind
		   structure is updated.  But relatively few photons hit the disk under normal situations, and therefore the
		   statistcs are not very good.  */
		    for (n = 0; n < NRINGS; n++)
		    {
		...
		/* Now execute each subcycle */
		    ...
		    photons_per_cycle is the number of photon bundles which will equal the luminosity; 0 => for ionization
		       calculation */
			...
			ztot += zz;
			//Total luminosity in all subcycles, used for calculating
			    disk heating

			/* kbf_need determines how many & which bf processes one needs to considere.  It was introduced as a way to
			   speed up the program.  It has to be recalculated evey time one changes freqmin and freqmax */
		...
		if (!keep_photoabs)
		{
		    ...
		    //Do not calculated photoabsorption in detailed spectrum
		}
	    ...
	    /* Calculate and store which bf processess need to be considered in each cell Note that this is not macro-specific but
	       is just to speed the program up. */

		kbf_need(freqmin, freqmax);

	    /* XXXX - BEGIN CYCLES TO CREATE THE DETAILED SPECTRUM */

	    /* the next section initializes the spectrum array in two cases, for the standard one where one is calulating the
	       spectrum for the first time and in the somewhat abnormal case where additional ionization cylcles were calculated
	       for the wind */
		...
		For the detailed spectra, NPHOT*pcycles is the number of photon bundles which will equal the luminosity, 1 implies
		   that detailed spectra, as opposed to the ionization of the wind is being calculated
		...
		*/
		...
		/* Write out the detailed spectrum each cycle so that one can see the statistics build up! */
		    ...
		    wind_save(windsavefile);
		    //This is only needed to update pcycle
			spec_save(specsavefile);
		    geo.pcycle++;
		    //Increment the spectal cycles
		...
		//Summarize the errors that were recorded by the program
...
**************************************************************/
	...
	//080518 - ksl - moved this up
	geo.adiabatic = 0;
	//Default is set so that adiabatic cooling is not included in the wind
	geo.auger_ionization = 1;
	//Default is on.


	geo.wind_type = 0;
	//Schlossman and Vitello
...
*/
    ...
    /* Next two lines are to allow for fact that photons generated in a frequency range may be Doppler shifted out of that range,
       ...
       0.02 to 0.03, e.g from 6000 km/s to 9000 km/s shift. */
#if DEBUG
	Log("photon_checks: %s\n", comment);
#endif
	freqmax *= (1.8);
...
**************************************************************/
    ...
    char            get_spectype_oldname[] = "kurucz91/kurucz91.ls";	/* This is to assure that we read model lists in the same
									   order everytime */
	...
	int             stype;
	int             get_models();
//Note:Needed because get_models cannot be included in templates.h
	    if (yesno)
	{
	...
	/* Now convert the response back to the values which python uses */
	    if (stype == 0)
		*spectype = SPECTYPE_BB;
	    //bb
		else if (stype == 2)
		*spectype = SPECTYPE_UNIFORM;
	    //uniform
		else
	    {
		if (geo.wind_type == 2)
		...
		strcpy(geo.model_list[get_spectype_count], model_list);
		//Copy it to geo
		    strcpy(get_spectype_oldname, model_list);
		//Also copy it back to the old name
		    get_spectype_count++;
	    }
	} else
	{
	    *spectype = SPECTYPE_NONE;
	    //No radiation
	}
...
**************************************************************/
	...
	for (n = 0; n < NRINGS; n++)
	{
	    ...
	    //theat is temperature if no
		internal energy production
		    if (qdisk.nhit[n] > 0)
		{

		    qdisk.ave_freq[n] /= qdisk.heat[n];
		    qdisk.t_hit[n] = H * qdisk.ave_freq[n] / (BOLTZMANN * 3.832);
