 ...
 Synopsis:
 	...
 	The main routines are
 	          ...
 	          pdf_gen_from_func	(&pdf,&func,xmin,xmax,njumps,jump)		
 	          	Generate a cumulative distrubution function (pdf) from a function
 	          
 	          pdf_gen_from_array(&pdf,x,y,n_xy,xmin,xmax,njumps,jump)		
 	          	Generate a cumulative distribution function (pdf) from an
 			array of values

 	double pdf_get_rand(&pdf)				
 			Generate a single sample from a pdf defined by either of the first
 			two routines 	
	...
	double pdf_get_rand_limit(&pdf) will samples the pdf but limits the range of
...
Arguments for pdf_gen_from_array are same as above except
	...
	double x[],y[]		The one dimensional arrays containg the places x where the probability
				density has been evaluated.  The array need not be uniformly spaced.  
...
Notes:
	...
	Error??: A shortcoming of this progam occurs when the probability density function
	is changing rapidly, e.g. at the tail end of a bb distribution, where the probability
	density declines.  In this regime the difference between points in the pdf_array
	will be large, but in fact the probability density is much less at the upper end
	of the interval than the lower end.  Unfortunately, what these programs do is
	to assume the probility denisty is constant within the interval, whereas in fact
	nearly all of the probability density is near the low edge.  Some kind of spline
	approch might deal with this. In the Numerical recipes routines, you develop
	the input data for the spline when the pdf is generated, and then you would have
	a relatively simple routine to get the photons frequencies, or whatever that
	you need.

	Error??: For Kurucz modles, The probability distribution beyond the He edge  
...
/* This routine simply allocates a working array */
int
pdf_init ()
{
  double d;
     ...
     int njumps;
{
  ...
  /* Check some of the input data before proceeding */
  if (xmax > xmin)
  ...
  for (n = 0; n < PDFSTEPS; n++)
	...
	{
	  idiag = 1;
	  Log ("ZOWIE  n %d z %g pdf_array[n] %g x %g\n", n, z, pdf_array[n],
	       x);
	  for (m = 0; m < n; m += 100)
	    {
	      z = (*func) (x);
	      Log ("zowie n %d x %g z %g pdf_array %g\n", m, x =
		   xmin + (0.5 * m) * xstep, z, pdf_array[m]);
	    }
	}
    }
     ...
     char filename[];
{
  ...
  fprintf (fptr, "#x y\n");
  for (n = 0; n <= NPDF; n++)
    fprintf (fptr, "%10.4g	%14.8g (%14.8e)\n", pdf->x[n], pdf->y[n],
	     1. - pdf->y[n]);
