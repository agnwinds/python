     ...
     double freq;               // Frequency of the current photon being tracked
{
  ...
  x = (sigma * PLANCK) / (MELEC * CSPEED * CSPEED);    //Calculate the constant
...
//            x += qromb (comp_cool_integrand, f1, f2, 1e-6);
  ...
  x *= (16. * PI * BOLTZMANN * t_e * xplasma->ne) / (MELEC * CSPEED * CSPEED) * xplasma->vol;     //Multply by the other terms - including temperature - this gives the temperature dependance of this cooling term.
     ...
     double nu;                 //The frequency of the photon packet
{
  ...
  x = (PLANCK * nu) / (MELEC * CSPEED * CSPEED);       //The photon energy relative to the rest mass energy of an electron
...

{
  ...
  x1 = PLANCK * p->freq / MELEC / CSPEED / CSPEED;     //compute the ratio of photon energy to electron energy. In the electron rest frame this is just the electron rest mass energy
