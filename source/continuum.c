     ...
     double t, g, freqmin, freqmax;
{
  ...
  if (old_t != t || old_g != g || old_freqmin != freqmin
      ...
      if (t != comp[spectype].xmod.par[0] || g != comp[spectype].xmod.par[1])
	{
      ...
      if (comp[spectype].xmod.w[0] < lambdamin
	  && lambdamin < comp[spectype].xmod.w[comp[spectype].nwaves - 1])
	{
	  w_local[nwave] = lambdamin;
	  linterp (lambdamin, comp[spectype].xmod.w, comp[spectype].xmod.f,
		   comp[spectype].nwaves, &y, 0);
	  f_local[nwave] = y;
	  nwave++;
	}
	...
	{
	  if (comp[spectype].xmod.w[n] > lambdamin
	      && comp[spectype].xmod.w[n] <= lambdamax)
	    {
      ...
      if (comp[spectype].xmod.w[0] < lambdamax
	  && lambdamax < comp[spectype].xmod.w[comp[spectype].nwaves - 1])
	{
	  w_local[nwave] = lambdamax;
	  linterp (lambdamax, comp[spectype].xmod.w, comp[spectype].xmod.f,
		   comp[spectype].nwaves, &y, 0);
	  f_local[nwave] = y;
	  nwave++;
	}
      ...
      if (f_local[nwave - 2] == 0.0)	//We have a zero just inside the end
	{
	  ...
	  linterp (w_local[nwave - 2], comp[spectype].xmod.w,
		   comp[spectype].xmod.f, comp[spectype].nwaves, &y, 0);
	  f_local[nwave - 2] = y;
	}
	  ...
	  (&comp[spectype].xcdf, w_local, f_local, nwave, lambdamin,
	   lambdamax) != 0)
	{
