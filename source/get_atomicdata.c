     ...
     char masterfile[];
{
  ...
  int index_collisions (), index_lines (), index_phot_top (),
    index_inner_cross (), index_phot_verner (), check_xsections ();
  int nwords;
  int nlte, nmax;
  ...
  for (n = 0; n < NIONS * N_INNER; n++)	//Initialise atomic arrasy with dimension NIONS*NINNER
    {
      ...
      inner_cross[n].nion = inner_elec_yield[n].nion =
	inner_fluor_yield[n].nion = (-1);
      inner_cross[n].n_elec_yield = -1;
      inner_cross[n].n_fluor_yield = -1;
      inner_cross[n].n = inner_elec_yield[n].n = inner_fluor_yield[n].n =
	(-1);
      inner_cross[n].l = inner_elec_yield[n].l = inner_fluor_yield[n].l =
	(-1);
      inner_cross[n].z = inner_elec_yield[n].z = inner_fluor_yield[n].z =
	(-1);
 ...
 * */
		case 'e':
		  if (sscanf
		      (aline, "%*s %d %s %le", &ele[nelements].z,
		       ele[nelements].name, &ele[nelements].abun) != 3)
		    {
		      Error
...
*/
		...
		case 'w':
		  if (strncmp (word, "PhotMacS", 8) == 0)
		      ...
		      if (ion[config[m].nion].phot_info == -1)
			{
		      ...
		      if (m ==
			  config[ion[config[n].nion].first_nlte_level].ilv)
			{
		      ...
		      if (n == nlevels)
			{
			  ...
			  Debug
			    ("No level found to match PhotTop data in file %s on line %d. Data ignored.\n",
			     file, lineno);
		  ...
		  /* Check that there is an ion which has the same ionization state as this record 
		      ...
		      for (nion = 0; nion < nions; nion++)
			{
			  if (ion[nion].z == z && ion[nion].istate == istate)
				   ...
				   JM 1508 -- don't do this with macro-atoms for the moment */
				{
				  ...
				  phot_top[ion[nion].ntop_ground].istate =
				    istate;
				  phot_top[ion[nion].ntop_ground].np = np;
				  phot_top[ion[nion].ntop_ground].nlast = -1;
				  phot_top[ion[nion].ntop_ground].macro_info =
				    0;
				  ...
				  if (phot_freq_min >
				      phot_top[ion[nion].ntop_ground].freq[0])
				    phot_freq_min =
				      phot_top[ion[nion].ntop_ground].freq[0];
				  Debug
			...
			("get_atomic_data: file %s line %d: photoionization line incorrectly formatted\n",
			 file, lineno);
		      Log
			("Make sure you are using the tabulated verner cross sections (photo_vfky_tabulated.data)\n");
		      Error ("Get_atomic_data: %s\n", aline);
		      exit (0);
		    }
		...
		case 'I':
		  if (sscanf
		      (aline, "%*s %d %d %d %d %le %d\n", &z, &istate, &in,
		       &il, &exx, &np) != 6)
		    {
		      Error
			("Inner shell ionization data incorrectly formatted\n");
		      Error ("Get_atomic_data: %s\n", aline);
		      exit (0);
		    }
		  for (n = 0; n < np; n++)
			...
			{
			  Error
			    ("Get_atomic_data: Problem reading VY inner shell record\n");
			  Error ("Get_atomic_data: %s\n", aline);
			  exit (0);
			}
		      sscanf (aline, "%*s %le %le", &xe[n], &xx[n]);
		  ...
		  for (nion = 0; nion < nions; nion++)
		      ...
		      if (ion[nion].z == z && ion[nion].istate == istate)
			{
			  ...
			  if (inner_freq_min >
			      inner_cross[n_inner_tot].freq[0])
		  ...
		  if (n_inner_tot > N_INNER * NIONS)
		    {
		      Error
			("getatomic_data: file %s line %d: Inner edges than we have room for.\n",
			 file, lineno);
		      exit (0);
		    }
		  break;
 ...
 */
		case 'x':	/*The line contains collision strength information--Gaetz & Salpeter */
		  if (sscanf
		      (aline, "%*s %*s %d %d %le %le %le %le %le", &z,
		       &istate, &exx, &lambda, &alpha, &beta, &tm) != 7)
		    {
		      Error
		  ...
		  for (n = 0; n < nions; n++)	//Loop over ions to find the correct place to put the data
		      ...
		      if (ion[n].z == z && ion[n].istate == istate)	// this works out which ion we are dealing with
			{
			  if (ion[n].dere_di_flag == 0)	//This is first set of this type of data for this ion
				...
				{
				  dere_di_rate[n_dere_di_rate].temps[n1] =
				    temp[n1];
				  dere_di_rate[n_dere_di_rate].rates[n1] =
				    temp[n1 + nspline] * 1e-6;
			    ...
			    {
			      Error
				("Get_atomic_data: More than one Dere DI rate for ion %i\n",
				 n);
			    }
			}
		    }
...
*/
		...
		case 'K':
		  ...
		  for (n = 0; n < n_inner_tot; n++)
		    {
		      if (inner_cross[n].z == z
			  && inner_cross[n].istate == istate
			  && inner_cross[n].n == in && inner_cross[n].l == il)
			{
			  if (inner_cross[n].n_elec_yield == -1)	/*This is the first yield data for this vacancy */
			    {
			      ...
			      inner_elec_yield[n_elec_yield_tot].istate =
				istate;
			      inner_elec_yield[n_elec_yield_tot].n = in;
			      inner_elec_yield[n_elec_yield_tot].l = il;
			      inner_elec_yield[n_elec_yield_tot].I =
				I * EV2ERGS;
			      inner_elec_yield[n_elec_yield_tot].Ea =
				Ea * EV2ERGS;
			      for (n1 = 0; n1 < 10; n1++)
				{
				  inner_elec_yield[n_elec_yield_tot].prob[n1]
				    = temp[n1] / 10000.0;
				}
			      n_elec_yield_tot++;
			    }
			    ...
			    {
			      Error
				("Get_atomic_data: more than one electron yield record for inner_cross %i\n",
				 n);
			    }
			}
		    }
 ...
 */
		case 'F':
		  nparam =
		    sscanf (aline, "%*s %d %d %d %d %le %le ", &z, &istate,
			    &in, &il, &energy, &yield);
		  if (nparam != 6)
		    {
			  ...
			  if (inner_cross[n].n_fluor_yield == -1)	/*This is the first yield data for this vacancy */
			    {
			      ...
			      inner_cross[n].n_fluor_yield =
				n_fluor_yield_tot;
			      inner_fluor_yield[n_fluor_yield_tot].z = z;
			      inner_fluor_yield[n_fluor_yield_tot].istate =
				istate;
			      inner_fluor_yield[n_fluor_yield_tot].n = in;
			      inner_fluor_yield[n_fluor_yield_tot].l = il;
			      inner_fluor_yield[n_fluor_yield_tot].freq =
				energy / HEV;
			      inner_fluor_yield[n_fluor_yield_tot].yield =
				yield;
			      n_fluor_yield_tot++;
			    }
			  else
		  ...
		  nparam =
		    (sscanf
		     (aline,
		      ...
		      &z, &istate, &freq, &f, &gl, &gu, &el, &eu, &levl,
		      &levu, &c_l, &c_u, &en, &gf, &hlt, &np, &type, &sp));
		  if (nparam != 18)
		    {
		      Error
			("Get_atomic_data: file %s line %d: Collision strength line incorrectly formatted\n",
			 file, lineno);
		      Error ("Get_atomic_data: %s\n", aline);
		      exit (0);
		    }
		  for (n = 0; n < nlines; n++)	//loop over all the lines we have read in - look for a match
		    {
			  ...
			  && line[n].levl == levl && line[n].levu == levu
			  && line[n].gl == gl && line[n].gu == gu
			  && line[n].f == f)
			{
			  if (line[n].coll_index > -1)	//We already have a collision strength record from this line - throw an error and quit
			    {
			      Error
				("Get_atomic_data More than one collision strength record for line %i\n",
				 n);
			      exit (0);
			    }
			  ...
			  if (fgets (aline, LINELENGTH, fptr) == NULL)
			    {
			      Error
				("Get_atomic_data: Problem reading collision strength record\n");
			      Error ("Get_atomic_data: %s\n", aline);
			      exit (0);
			    }
...
/* OK now summarize the data that has been read*/
  ...
  for (n = 0; n < n_inner_tot; n++)
	...
	Error
	  ("get_atomicdata: No inner electron yield data for inner cross section %i\n",
	   n);
  ...
  if (write_atomicdata)
      ...
      for (n = 0; n < nions; n++)
	{
	  fprintf (fptr,
		   ...
		   n, ion[n].z, ion[n].istate, ion[n].firstlevel,
		   ion[n].nlevels, ion[n].ip / EV2ERGS);
	}
      ...
      for (n = 0; n < ntop_phot + nxphot; n++)
	{
	  fprintf (fptr, "n %3d z %2d istate %3d sigma %8.2e freq[0] %8.2e\n",
		   n, phot_top[n].z, phot_top[n].istate, phot_top[n].sigma,
		   phot_top[n].freq[0]);
	}
      ...
      for (n = 0; n < nlines; n++)
	{
	  fprintf (fptr, "n %3d ion %3d freq %8.1e f %6.3f\n", n,
		   line[n].nion, line[n].freq, line[n].f);
	}
      ...
      for (n = 0; n < NIONS; n++)
	{
	  fprintf (fptr, "%3d %3d %6.3f %6.3f\n", ground_frac[n].z,
		   ground_frac[n].istate, ground_frac[n].frac[0],
      ...
      fprintf (fptr,
	       "Collision strengths: There were %d collision strengths\n",
	       nxcol);
  ...
  for (n = 0; n < nphot_total; n++)
	...
	Debug
	  ("Topbase Ion %i Z %i istate %i nground %i ilv %i ntop %i f0 %8.4e IP %8.4e\n",
	   nion, ion[nion].z, ion[nion].istate, ion[nion].ntop_ground,
	   phot_top[n].nlev, ion[nion].ntop, phot_top[n].freq[0],
	   ion[nion].ip);
      else if (ion[nion].phot_info == 0)
	Debug ("Vfky Ion %i Z %i istate %i nground %i f0 %8.4e IP %8.4e\n",
	       nion, ion[nion].z, ion[nion].istate, ion[nion].nxphot,
	       phot_top[n].freq[0], ion[nion].ip);
