...
{
  zdom[ndom].wind_rmin_at_disk_plane = zdom[ndom].rho_min = 0;
  zdom[ndom].rmin = imported_model[ndom].r[1];  // <- this assumes the 1st cell is a ghost cell
  zdom[ndom].wind_rmax_at_disk_plane = zdom[ndom].zmax = zdom[ndom].rho_max = zdom[ndom].rmax = imported_model[ndom].r[imported_model[ndom].ncell - 2]; // <- this assumes the last 2 cells are ghost cells
