...

{
  ...
  while (niterate < MAXITERATIONS)
  {
    ...
    for (nn = 0; nn < nions; nn++)
    {
      ...
      if ((ion[nn].macro_info == TRUE) && (geo.macro_simple == 0) && (geo.macro_ioniz_mode == MACRO_IONIZ_MODE_ESTIMATORS))
      {
...
/* We need to get the 'true' new electron density so we need to do a little loop here to compute it */
  ...
  for (nn = 0; nn < nions; nn++)
  {
    ...
    if (ion[nn].macro_info == FALSE || geo.macro_ioniz_mode == MACRO_IONIZ_MODE_NO_ESTIMATORS || geo.macro_simple == 1)
    {
