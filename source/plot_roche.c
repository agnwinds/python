     ...
     char *argv[];
{
  ...
  printf ("This program simulates radiative transfer in a (biconical) CV or (spherical) stellar wind\n");
  ...
  {
    printf ("Proceeding to create rdpar file in dummy.pf, but will not run prog\n");
  }
...
/* Gather input data */
  ...
  if (geo.bl_radiation)
  {
    xbl = geo.lum_bl = 0.5 * G * geo.mstar * geo.disk_mdot * MSOL * MSOL / (geo.rstar * YR);
...
/* Describe the spectra which will be extracted and the way it will be extracted */
  ...
  {
    if (geo.star_radiation)
      rdint ("Rad_type_for_star(0=bb,1=uniform,2=Kur)_in_final_spectrum", &geo.star_spectype);
    if (geo.disk_radiation)
      rdint ("Rad_type_for_disk(0=bb,1=uniform,2=Kur)_in_final_spectrum", &geo.disk_spectype);
    if (geo.bl_radiation)
      rdint ("Rad_type_for_bl__(0=bb,1=uniform,2=Kur)_in_final_spectrum", &geo.bl_spectype);
...
init_geo ()
{
  ...
  geo.star_ion_spectype = geo.star_spectype = geo.disk_ion_spectype = geo.disk_spectype = geo.bl_ion_spectype = geo.bl_spectype = 0;
