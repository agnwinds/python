...
int get_proga_wind_params(void)
{
	...
	Log("Creating a wind model using a Proga Zeus Calc\n");
	...
	geo.wind_rmax = geo.rmax = proga_r_edge[iproga_r] + proga_dr_edge[iproga_r];	// Set the outer edge of the wind to the outer
																																								// edge of the final defined cell
	Log("rmax=%e\n", geo.rmax);
...
int get_proga(void)
{
	...
	if (proga_ptr == NULL)
	{
		Error("There is a problem in allocating memory for the proga structure\n");
		exit(0);
	}
	...
	{
		itest = sscanf(aline, "%d %lf %lf %lf %lf", &i, &r_edge, &r, &dr_edge, &dr);	/* NSH 130322 - minor mod here - it is
																																									   actually the second value which is the
																																									   centre of the cell, where the data is
																																									   defined so we ignore the first */
	...
	/* NSH 130327 - Added a parameter to define the angle at which the disk is defined This allows one to disregard theta cells
	   which contain the disk in Daniels model */
	...
	while (fgets(aline, LINE, fptr) != NULL)
	{
		itest = sscanf(aline, "%d %lf %lf %lf %lf", &i, &theta_edge, &theta, &dtheta_edge, &dtheta);	/* NSH 130322 - minor mod
																																																	   here - it is actually the
																																																	   second value which is the
																																																	   centre of the cell, where
																																																	   the data is defined so we
																																																	   ignore the first */
		if (itest != 5)							// We have an line which does not match what we expect, so quit
		{
	...
	{
		// printf ("PROGA j_proga_thetamax=%i, bracketing cells have theta = %f and
	...
	while (fgets(aline, LINE, fptr) != NULL)
	{
		...
		if (itest == 7)
		{
			...
			if (j > ithetamax)
				ithetamax = j;
			/* NSH 130327 - for the time being, if theta is in the disk, replace with the last density above the disk */
			proga_ptr[i * MAXPROGA + j].temp = ((2.0 / 3.0) * energy) / ((rho / MPROT) * BOLTZMANN);	// Work out the temperature
	...
	{
		NDIM = geo.ndim = iproga_r - IGHOST + 3;	// We need an inner cell to bridge the star and the indisde of the wind, and an
																							// outer cell
...
double proga_velocity(double x[], double v[])
{
	...
	while (proga_r_cent[ii] < r && ii <= iproga_r)	// Search through radius array, until array value is greater than your value
																									// of r
		ii++;
	while (proga_theta_cent[jj] < theta && jj <= iproga_theta)	// Search through theta array until value is greater than your
																															// value of theta
		jj++;
	...
	{															// r is in the normal range
		...
		f1 = (r - proga_r_cent[ii - 1]) / (proga_r_cent[ii] - proga_r_cent[ii - 1]);	// Work out fractional position in the ii-1th 
																																									// radial bin where you want to be
	...
	{															// theta is inside the normal range
		...
		f2 = (theta - proga_theta_cent[jj - 1]) / (proga_theta_cent[jj] - proga_theta_cent[jj - 1]);
		jm = jj - 1;
	}
								 ...
								 f2 * proga_ptr[im * MAXPROGA + jj].v[0]) + f1 * ((1. -
																																	...
																																	proga_ptr[ii
																																						*
																																						MAXPROGA
																																						+
																																						jm].v[0] + f2 * proga_ptr[ii * MAXPROGA + jj].v[0]);
								 ...
								 f2 * proga_ptr[im * MAXPROGA + jj].v[1]) + f1 * ((1. -
																																	...
																																	proga_ptr[ii
																																						*
																																						MAXPROGA
																																						+
																																						jm].v[1] + f2 * proga_ptr[ii * MAXPROGA + jj].v[1]);
								 ...
								 f2 * proga_ptr[im * MAXPROGA + jj].v[2]) + f1 * ((1. -
																																	...
																																	proga_ptr[ii
																																						*
																																						MAXPROGA
																																						+
																																						jm].v[2] + f2 * proga_ptr[ii * MAXPROGA + jj].v[2]);
		...
		(1. - f1) * ((1. - f2) * proga_ptr[im * MAXPROGA + jm].rho +
								 f2 * proga_ptr[im * MAXPROGA + jj].rho) + f1 * ((1. -
																																 ...
																																 proga_ptr[ii
																																					 *
																																					 MAXPROGA
																																					 + jm].rho + f2 * proga_ptr[ii * MAXPROGA + jj].rho);
								 ...
								 f2 * proga_ptr[im * MAXPROGA + jj].temp) + f1 * ((1. -
																																	...
																																	proga_ptr[ii
																																						*
																																						MAXPROGA
																																						+
																																						jm].temp + f2 * proga_ptr[ii * MAXPROGA + jj].temp);
...
int rtheta_make_zeus_grid(WindPtr w)
{
	...
	for (i = 0; i < NDIM; i++)
	{
		for (j = 0; j < MDIM; j++)
		{
			...
			else if (i + IGHOST - 1 > iproga_r)	// We are at the radial limit of the data, this last cell will be a ghost cell of our 
																					// own
			{
			...
			{
				w[n].r = proga_r_edge[i + IGHOST - 1];	// The -1 is to take account of the fact that i=0 cell is the inner part of the 
																								// geometry inside the wind
				w[n].rcen = proga_r_cent[i + IGHOST - 1];
			}
			if (proga_theta_cent[j + IGHOST] + (proga_dtheta_cent[j + IGHOST] / 2.0) > proga_thetamax && proga_theta_cent[j + IGHOST] - (proga_dtheta_cent[j + IGHOST] / 2.0) < proga_thetamax)	// This 
																																																																																													// cell 
																																																																																													// bridges 
																																																																																													// the 
																																																																																													// boundary
			...
			// printf ("Cell %i r=%e, rcne=%e, theta=%f, thetacen=%f, x=%e, y=%e, z=%e,
			// inwind=%i\n",n,w[n].r,w[n].rcen,w[n].theta,w[n].thetacen,w[n].x[0],w[n].x[1],w[n].x[2],w[n].inwind);

		}
	}

	/* for (i = 0; i < NDIM; i++) { wind_ij_to_n (i, 0, &n); printf ("i=%i, n=%i, r=%e, rcen=%e\n",i,n,w[n].r,w[n].rcen); }

	   for (i = 0; i < MDIM; i++) { wind_ij_to_n (0, i, &n); printf ("j=%i, iprogatheta=%i, n=%i, theta=%f,
	...
	/* 
	   cones_rtheta = (ConePtr) calloc (sizeof (cone_dummy), MDIM); if (cones_rtheta == NULL) { Error ("rtheta_make_grid: There
	   is a problem in allocating memory for the cones structure\n"); exit (0);

	   }


	   for (n = 0; n < MDIM; n++) { cones_rtheta[n].z = 0.0; cones_rtheta[n].dzdr = 1. / tan (w[n].theta / RADIAN); // New
	   definition } */
