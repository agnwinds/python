...
/*Write something into the file name strings */
  ...
  while (fgets (aline, LINE, fptr) != NULL)
    {
      itest = sscanf (aline, "%d %lf %lf %lf %lf", &i, &r_edge, &r, &dr_edge, &dr);	/*NSH 130322 - it is the second value which is the centre of the cell, where the data is defined so we ignore the first */
      if (itest != 5)		//We have an line which does not match what we expect, so quit
	{
	  Error
	    ("proga.c radius file improperly formatted, check for headers and footers\n");
	  exit (0);
	}
  ...
  if (proga_r_edge[IGHOST] < geo.rstar)
    {
      Error
	("Major problem, innermost edge of proga radial grid begins inside geo.rstar\n");
      exit (0);
    }
...
This allows one to disregard theta cells which contain the disk in Daniels model */
  ...
  while (fgets (aline, LINE, fptr) != NULL)
    {
      itest = sscanf (aline, "%d %lf %lf %lf %lf", &i, &theta_edge, &theta, &dtheta_edge, &dtheta);	/*NSH 130322 - it is the second value which is the centre of the cell, where the data is defined so we ignore the first */
      if (itest != 5)		//We have an line which does not match what we expect, so quit
	{
	  Error
	    ("proga.c theta file improperly formatted - check for headers and footers\n");
	  exit (0);
	}
      ...
      if (proga_theta_edge[i] > proga_thetamax
	  && proga_theta_edge[i - 1] <= proga_thetamax)
	{
...
/* NSH 130327 - for the time being, if theta is in the disk, replace with the last
 density above the disk */
	...
	{
	  Error
	    ("Proga data file not correctly formatted - check for headers and footers\n");
	  exit (0);
	}
     ...
     double x[];
{
  ...
  if (ii > 0)
    {
      f1 =
	(r - proga_r_cent[ii - 1]) / (proga_r_cent[ii] -
				      proga_r_cent[ii - 1]);
      im = ii - 1;
    }
  else
...
/***********************************************************
 ...
 Synopsis:
	rtheta_make_zeus_grid defines the cells in a rtheta grid based upon 
	the coordinates that can be read in from a zeus (the hydrocode used by Proga for some simulations)            
  ...
  for (i = 0; i < NDIM; i++)
      ...
      for (j = 0; j < MDIM; j++)
	{
	  ...
	  if (proga_theta_cent[j + IGHOST] + (proga_dtheta_cent[j + IGHOST] / 2.0) > proga_thetamax && proga_theta_cent[j + IGHOST] - (proga_dtheta_cent[j + IGHOST] / 2.0) < proga_thetamax)	//This cell bridges the boundary
	    {
	      theta =
		proga_theta_cent[j + IGHOST - 1] +
		(proga_theta_cent[j + IGHOST - 1] -
		 proga_theta_cent[j + IGHOST - 2]) / 2;
