  ...
  while (geo.wcycle < geo.wcycles)
    {				/* This allows you to build up photons in bunches */
      ...
      spectrum_init (freqmin, freqmax, geo.nangles, geo.angle, geo.phase,
		     geo.scat_select, geo.top_bot_select, geo.select_extract,
		     geo.rho_select, geo.z_select, geo.az_select,
		     geo.r_select);
	  ...
	  || (geo.wcycle == 0 && geo.run_type != RUN_TYPE_PREVIOUS))
	...
	pop_kappa_ff_array ();
      ...
      zze = zzz = zz_adiab = zz_abs = zz_scat = zz_star = zz_disk = zz_err =
	zz_else = 0.0;
      nn_adiab = 0;
      for (nn = 0; nn < NPHOT; nn++)
	{
	...
	Log
	  ("!!python: luminosity lost by adiabatic kpkt destruction %18.12e number of packets %d\n",
	   zz_adiab, nn_adiab);
      Log
	("!!python: luminosity lost by being completely absorbed  %18.12e \n",
	 zz_abs);
      Log
	("!!python: luminosity lost by too many scatters          %18.12e \n",
	 zz_scat);
      Log
	("!!python: luminosity lost by hitting the star           %18.12e \n",
	 zz_star);
      Log
	("!!python: luminosity lost by hitting the disk           %18.12e \n",
	 zz_disk);
      Log
	("!!python: luminosity lost by errors                     %18.12e \n",
	 zz_err);
      Log
	("!!python: luminosity lost by the unknown                %18.12e \n",
	 zz_else);
 ...
 * file. */
      ...
      if (rank_global == 0)
	{
	  ...
	  if (modes.make_tables)
	    {
	      strcpy (dummy, "");
	      sprintf (dummy, "diag_%s/%s%02d", files.root, files.root,
		       geo.wcycle);
...
/* Write out the detailed spectrum each cycle so that one can see the statistics build up! */
      ...
      if (rank_global == 0)
	{
	  ...
	  spectrum_summary (files.spec, 0, nspectra - 1, geo.select_spectype,
			    renorm, 0, 0);
	  spectrum_summary (files.lspec, 0, nspectra - 1, geo.select_spectype,
			    renorm, 1, 0);

	  /* Next lines  produce spectra from photons in the wind only */
	  spectrum_summary (files.spec_wind, 0, nspectra - 1,
			    geo.select_spectype, renorm, 0, 1);
	  spectrum_summary (files.lspec_wind, 0, nspectra - 1,
			    geo.select_spectype, renorm, 1, 1);
