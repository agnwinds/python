...
{
  int rc;
  //Return code from running Setup_Py_Dir
  struct stat file_stat;
  //Used to check the atomic data exists
  ...
  /*
   * 181010-ksl-There a several line transfer modes which are
   * diagnostic in nature which cannot be reached by rdchoice easily,
   * except as numbers.  We need a better way to deal with these.
   */
  ...
  /*
   * JM 1406 -- geo.rt_mode and geo.macro_simple control different
   * things. geo.rt_mode controls the radiative transfer and whether or
   * not you are going to use the indivisible packet constraint, so you
   * can have all simple ions, all macro-atoms or a mix of the two.
   * geo.macro_simple just means one can turn off the full macro atom
   * treatment and treat everything as 2-level simple ions inside the
  ...
  else if (user_line_mode == 5)
  {
    ...
    geo.scatter_mode = SCATTER_MODE_THERMAL;
  ...
  else if (user_line_mode == 8)
  {
    ...
    geo.line_mode = LINE_MODE_ESC_PROB;
    geo.rt_mode = RT_MODE_MACRO;
    //Identify macro atom treatment i.e.indivisible packets
    geo.macro_simple = TRUE;
    //This is for test
  ...
  /*
   * read in variables to set the transition
   * mode form macro-atoms
   */
  /* an adaptive mode might be added in future */
  if (geo.rt_mode == RT_MODE_MACRO)
  {
    /*
     * XMACRO -- these options MUST be
     * consistent with the define
     * statements in python.h
     */
    ...
    if (geo.run_type == RUN_TYPE_PREVIOUS)
    {
      ...
      /*
       * With the macro atom
       * approach we won't want to
       * generate photon bundles in
       * the wind so switch it off
       * here. If not in macro atom
       * mode ask the user whether
       * we want the wind to
       * radiate
       */
      if (geo.rt_mode == RT_MODE_MACRO)
      ...
      /*
       * Note the only other
       * variable read in in this
       * section is that for the
       * atomic data, which can
       * only be specified for a
       * new model, becuase of
