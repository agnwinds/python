...
free (commbuffer);
  ...
  if (modes.zeus_connect==1) //If we are running in zeus connect mode - we open a file for heatcool rates
  {
	 ...
	 fprintf(fptr,"i j rcen thetacen heat_photo heat_comp heat_lines heat_ff lum_fb lum_comp lum_lines lum_ff xi rho vx vy vz temp nphot vol nh1 nh2\n");
...
// OLD           log10(plasmamain[nplasma].ferland_ip),log10(plasmamain[nplasma].ip)); 
	  ...
	  if (modes.zeus_connect==1) //If we are running in zeus connect mode, we output heating and cooling rates.
	  {
			 ...
			 vol=w[plasmamain[nplasma].nwind].vol;
	 		 fprintf(fptr,"%d %d %e %e %e %e %e %e %e %e %e %e %e %e %e %e %e %e %i %e %e %e\n",i,j,w[plasmamain[nplasma].nwind].rcen,
			 ...
			 plasmamain[nplasma].ntot,volume,plasmamain[nplasma].density[0],plasmamain[nplasma].density[1]);
	   }
    }
